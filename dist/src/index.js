...
const platformEnum = ["tiktok", "reels", "shorts", "linkedin", "x"];
const formatEnum = [
    "silent-demo",
    "screen-tutorial",
    "carousel",
...
const ensureMaxLen = (value, max, field) => {
    if (value.length > max) {
        throw new Error(`${field} exceeds ${max} characters`);
    }
};
const makeTextResult = (obj) => ({
    content: [{ type: "text", text: JSON.stringify(obj, null, 2) }]
});
const resolveConfig = (api) => {
    const fullConfig = typeof api?.getConfig === "function" ? api.getConfig() : undefined;
    return (fullConfig?.plugins?.entries?.trendpilot?.config ??
        api?.config ??
        {});
};
const getYouTubeTrends = async (config, regionCode, fetchFn) => {
...
const buildHooks = (niche, audience, seed) => {
    const templates = [
...
const buildShots = (niche, platform) => [
...
const buildOnScreenText = (niche, goal) => [
    `${niche} quick win`,
    `Do this for ${goal}`,
    `Step-by-step`,
    `Save for later`
];
const buildStructure = (goal) => [
    "Hook",
    "Key idea",
    "Steps",
    "Outcome",
...
const limitArray = (arr, count) => arr.slice(0, count);
export const createTrendpilotTools = (config, fetchFn) => {
    ...
    const trendpilot_brief = {
        ...
        execute: async (input) => {
            ...
            const regionCode = safeString(input?.regionCode ?? config.youtubeRegionCode ?? "US");
